...
def fetchone(q,p=()): c=get_conn();cur=c.cursor();cur.execute(q,p);r=cur.fetchone();cur.close();return r
def fetchall(q,p=()): c=get_conn();cur=c.cursor();cur.execute(q,p);r=cur.fetchall();cur.close();return r
def execute(q,p=()): c=get_conn();cur=c.cursor();cur.execute(q,p);c.commit();cur.close()

# resets
def reset_all_data():
    execute("DELETE FROM trades"); execute("DELETE FROM preferences")
    execute("DELETE FROM endowments"); execute("DELETE FROM users"); execute("VACUUM")
...
def get_group_users(grp): return fetchall("SELECT id,name FROM users WHERE grp=? ORDER BY name",(grp,))
def get_user_endowment(uid): r=fetchone("SELECT good FROM endowments WHERE user_id=?",(uid,));return r[0] if r else None
...
def propose_trade(grp,pid,rid):
    if pid==rid: return "Cannot trade with yourself."
    pg,rg=get_user_endowment(pid),get_user_endowment(rid)
    if not (pg and rg): return "Endowment missing."
    row=fetchone("""SELECT id FROM trades WHERE grp=? AND proposer_id=? AND recipient_id=? 
                    AND proposer_good=? AND recipient_good=? AND status='pending'""",
                    (grp,pid,rid,pg,rg))
    if row: return "Trade already pending."
    execute("""INSERT INTO trades (grp,proposer_id,recipient_id,proposer_good,recipient_good,status)
               VALUES (?,?,?,?,?,'pending')""",(grp,pid,rid,pg,rg))
    return "Trade proposed."

def incoming_trades(uid): 
    return fetchall("""SELECT t.id,u.name,t.proposer_good,t.recipient_good,t.status,t.ts
                       FROM trades t JOIN users u ON u.id=t.proposer_id
                       WHERE t.recipient_id=? AND t.status='pending'""",(uid,))
def outgoing_trades(uid):
    return fetchall("""SELECT t.id,v.name,t.proposer_good,t.recipient_good,t.status,t.ts
                       FROM trades t JOIN users v ON v.id=t.recipient_id
                       WHERE t.proposer_id=? AND t.status='pending'""",(uid,))
def update_trade_status(tid,status): execute("UPDATE trades SET status=? WHERE id=?",(status,tid))

def accept_trade(tid):
    r=fetchone("SELECT grp,proposer_id,recipient_id,proposer_good,recipient_good,status FROM trades WHERE id=?",(tid,))
    if not r: return "Trade not found."
    ...
    if get_user_endowment(pid)!=pg or get_user_endowment(rid)!=rg:
        update_trade_status(tid,"declined"); return "Offer invalid."
    execute("UPDATE endowments SET good=? WHERE user_id=?",(rg,pid))
    execute("UPDATE endowments SET good=? WHERE user_id=?",(pg,rid))
    update_trade_status(tid,"accepted"); return "Trade accepted and executed."
def decline_trade(tid): update_trade_status(tid,"declined")
def cancel_outgoing(tid,uid):
    r=fetchone("SELECT proposer_id,status FROM trades WHERE id=?",(tid,))
    if r and r[0]==uid and r[1]=="pending": update_trade_status(tid,"cancelled")

def total_current_utility(grp):
    r=fetchall("""SELECT p.utility FROM users u JOIN endowments e ON e.user_id=u.id 
                  JOIN preferences p ON p.user_id=u.id AND p.good=e.good WHERE u.grp=?""",(grp,))
...
def optimal_assignment(grp):
    ...
    n=len(df)
    if n==0: return 0,df,[]
    items=list(df["Good"]); util=preferences_matrix(grp,df,items)
    try:
...
# ── Social Planner Role ──────────────────────
    ...
    df=current_allocation_for_group(grp)
    if df.empty: st.warning("No students yet."); st.stop()
